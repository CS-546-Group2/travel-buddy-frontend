    ...
    document.getElementById('generate-ai-content').addEventListener('click', async function () {
      try {
        ...
        if (hasExistingContent) {
          ...
          if (trip.activities && Array.isArray(trip.activities) && trip.activities.length > 0) {
          ...
          if (trip.recommendations && (
            ...
            (trip.recommendations.restaurants && trip.recommendations.restaurants.length > 0) ||
            (trip.recommendations.experiences && trip.recommendations.experiences.length > 0)
          )) {
            existingContentTypes.push('Recommendations');
          }
          if (trip.travelTips && Array.isArray(trip.travelTips) && trip.travelTips.length > 0) {
        ...
        const generateBtn = document.getElementById('generate-ai-content');
        generateBtn.innerHTML = '<i class="fas fa-robot"></i><span>Generate AI Content</span>';
        generateBtn.disabled = false;
      }
    });
...
function hasAnyAIContent(trip) {
  return trip.activities && Array.isArray(trip.activities) && trip.activities.length > 0;
}
...
function displayExistingAIContent(trip) {
  ...
  if (trip.activities) {
    try {
      ...
      if (activities && activities.length > 0) {
        let itineraryHtml = '<div class="ai-section"><h3><i class="fas fa-calendar-alt"></i> Itinerary</h3>';
  ...
  if (trip.recommendations) {
    try {
      const recommendations = (typeof trip.recommendations === 'string') ? JSON.parse(trip.recommendations) : trip.recommendations;
      if (recommendations && (recommendations.attractions || recommendations.restaurants || recommendations.experiences)) {
        let recsHtml = '<div class="ai-section"><h3><i class="fas fa-star"></i> Recommendations</h3>';
        
        if (recommendations.attractions && recommendations.attractions.length > 0) {
          recsHtml += '<div class="recommendation-category"><h4><i class="fas fa-map-marker-alt"></i> Attractions</h4>';
        ...
        if (recommendations.restaurants && recommendations.restaurants.length > 0) {
          recsHtml += '<div class="recommendation-category"><h4><i class="fas fa-utensils"></i> Restaurants</h4>';
        ...
        if (recommendations.experiences && recommendations.experiences.length > 0) {
          recsHtml += '<div class="recommendation-category"><h4><i class="fas fa-heart"></i> Experiences</h4>';
  ...
  if (trip.travelTips) {
    try {
      const travelTips = (typeof trip.travelTips === 'string') ? JSON.parse(trip.travelTips) : trip.travelTips;
      if (travelTips && travelTips.length > 0) {
        let tipsHtml = '<div class="ai-section"><h3><i class="fas fa-lightbulb"></i> Travel Tips</h3>';
