const LOG_LEVELS = {
  ...
  let currentLogLevel = parseInt(localStorage.getItem('logLevel')) || LOG_LEVELS.INFO.level;
  ...
  const logger = {
    ...
    debug: (message, meta) => {
      if (shouldLog('DEBUG')) {
        const { message: formattedMsg, style } = formatMessage('DEBUG', message, meta);
    ...
    info: (message, meta) => {
      if (shouldLog('INFO')) {
        const { message: formattedMsg, style } = formatMessage('INFO', message, meta);
    ...
    warn: (message, meta) => {
      if (shouldLog('WARN')) {
        const { message: formattedMsg, style } = formatMessage('WARN', message, meta);
    ...
    error: (message, meta) => {
      if (shouldLog('ERROR')) {
        const { message: formattedMsg, style } = formatMessage('ERROR', message, meta);
    ...
    fatal: (message, meta) => {
      if (shouldLog('FATAL')) {
        const { message: formattedMsg, style } = formatMessage('FATAL', message, meta);
        console.error(`%c${formattedMsg}`, style);
      }
    }
  };
