...
document.addEventListener('DOMContentLoaded', function () {
  ...
  if (user.travelPreferences.accommodationStyle) {
    const travelAccommodationSelect = document.getElementById('select-accommodation-style');
  ...
  user.travelPreferences.interests.forEach(interest => {
      ...
      const checkbox = document.querySelector(`#select-interests input[type="checkbox"][value="${interest}"]`);
      if (checkbox) {
...
document.getElementById('preferencesForm').addEventListener('submit', async (e) => {
  ...
  const data = {
    travelPreferences: {
      ...
      interests: Array.from(form.querySelectorAll('#select-interests input[type="checkbox"]:checked')).map(i => i.value)
    }
  };

  try{
    const response = await fetch(`${appConfig.API_BASE}/users/profile/${userId}`, {
      method: 'PUT',
